...
echo "=== Configurando serviço systemd para Besu - CORREÇÃO DEFINITIVA NOEXEC ==="
...
# 1. CRIAR DIRETÓRIO TEMPORÁRIO EXECUTÁVEL PARA BESU
echo ""
echo "=== Criando diretório temporário executável ==="
sudo mkdir -p "$BESU_TEMP_DIR"
sudo chmod 755 "$BESU_TEMP_DIR"
echo "✅ Diretório temporário criado: $BESU_TEMP_DIR"
...
# 4. CRIAR/VERIFICAR USUÁRIO BESU PRIMEIRO
echo ""
...
# Configurar permissões do diretório temporário para o usuário besu
sudo chown besu:besu "$BESU_TEMP_DIR"
sudo chmod 755 "$BESU_TEMP_DIR"

# 5. CRIAR ARQUIVO DE SERVIÇO SYSTEMD SIMPLIFICADO E EFICAZ
echo ""
echo "=== Criando arquivo de serviço systemd ==="

sudo tee /etc/systemd/system/besu-node.service << EOF
[Unit]
Description=Hyperledger Besu Node - Bradesco Network (noexec fix)
...
# 7. CONFIGURAR SELINUX (se necessário)
...
sudo -u besu test -w "$BESU_TEMP_DIR" && echo "✅ Diretório temp acessível pelo usuário besu" || echo "❌ Problema com diretório temp"

# Teste crítico: verificar se pode criar arquivos executáveis no temp dir
sudo -u besu bash -c "
    echo '#!/bin/bash' > '$BESU_TEMP_DIR/test_exec'
    echo 'echo test' >> '$BESU_TEMP_DIR/test_exec'
    chmod +x '$BESU_TEMP_DIR/test_exec'
    if '$BESU_TEMP_DIR/test_exec' >/dev/null 2>&1; then
        echo '✅ Diretório temp permite execução'
        rm -f '$BESU_TEMP_DIR/test_exec'
    else
        echo '❌ Diretório temp NÃO permite execução - PROBLEMA!'
    fi
"
...
echo ""
echo "=== Verificação noexec ==="
echo "Para verificar se resolveu o problema:"
echo "sudo journalctl -u besu-node -f | grep -E 'tmp|temp|rocksdbjni|UnsatisfiedLinkError'"
echo ""
